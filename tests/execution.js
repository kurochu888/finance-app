...
let prev = { a: A.computeTrend(a), b: A.computeTrend(b) };
  ...
  if (ch.length){
    ...
    if (plan && /不可靠|歷史價格還不夠|超過「部位市值/.test(card) === false){
      if (plan.deltaLoan < -1){
        ...
        // 一半的時候照方案B(賣了還房貸),一半照方案A(賣了留現金);方案A 到不了就用 B
        // 像真人一樣只看卡片上寫的金額:卡片有「方案A:賣掉、錢留著」就照那個數字;沒有的話只能照唯一顯示的金額賣
        const cardTxt = card.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
        const mA = /方案A:賣掉、錢留著當現金 約 NT\$ ([\d,]+)/.exec(cardTxt);
        const keepCash = rnd() < 0.5 && !/全部賣掉也降不到目標/.test(cardTxt);
        const total = keepCash && mA ? Number(mA[1].replace(/,/g, '')) : Math.abs(plan.deltaLoan);
        let proceeds = 0;
        ...
        if (!keepCash){   // 還款:從最早的動用記錄開始還
          ...
          if (left > 1 && A.state.leverage.draws.length) {}   // 沒有借款可還的部分就留著
        }
      }else if (plan.deltaLoan > 1 && (plan.progress <= plan.layerCount || plan.holdAdjustPending)){
        const got = buy(a, plan.deltaLoan / 2, d, 'loan') + buy(b, plan.deltaLoan / 2, d, 'loan');
...
for (const sd of (process.argv[2] ? [Number(process.argv[2])] : [1, 2, 3])) runSeed(sd).forEach(p => bugs.push(p));
