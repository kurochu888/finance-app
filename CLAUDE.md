...
7. **歷史大空頭檢視(2026-09,結論:策略跟參數都不動)**——用「指定期間」看了五段(預設參數,使用者手機上的真實資料;
   ...
   - **進行中的實驗:FLOOR_STOP「接完再跌 X% 停損」(2026-09-25)**——使用者問 2008 有沒有解法,做成回測實驗
     (回測卡 `FLOOR_STOP` 區塊,X = 15~35%,runBacktest 只有傳 floorStop 才會觸發 STOP,實際訊號不動)。
     還原點分支 `restore/pre-floorstop-experiment-2026-09-25`。結果還沒看,看完要把結論補在這裡(採用/放棄)。
