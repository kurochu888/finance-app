...
6. **進行中:另外兩種高檔處理的回測實驗(2026-09)**——`runBacktest(hist, params, exp)` 的 `exp`
   (`EXIT_EXPERIMENTS`):實驗1 `trail` 高點回落 X% 全出、創新高再進場、先跌破出場線就接回 WAIT_RECOVER;
   實驗2 `overheat` 快線 1.3 倍賣一半、**收盤高過賣出價就買回**(針對第 5 點「賣了一路漲只好買更貴」的缺點)
   或回到快線 1.1 倍以內買回。另有對照組 `partial`(規則不變、每次進場只買到總資產 70/80/90%,
   續抱不再平衡,等於調低 HOLD 曝險),用來判斷實驗換到的回撤是不是「少抱一點」就做得到。
   回測畫面有門檻比較表(含年化÷MDD、以 2021 年初為界的前後段年化÷MDD)跟每次出場明細。
   第一輪真實資料(1.25~1.40):實驗1 全部比原策略差;實驗2 減碼門檻越高越好,1.40 那列兩檔年化÷MDD
   約 1.41~1.56(原策略 1.20/1.22),00675L 1.40/1.10 總報酬甚至略高於原策略;買回門檻幾乎不影響;
   對照組只投入 70~90% 全部比原策略差(續抱不再平衡,上漲後權重又漲回去)。好處集中在 2024-08、
   2026-03、2026-07 幾次急跌前剛好減碼。第二輪把減碼門檻延伸到 1.35~1.60,確認 1.40 是不是穩定區域,
   並看前後段是否都成立。一樣只在回測,
   `computeTrend` 沒有這些狀態,`exp` 為 null 時跟 `computeTrend` 逐行一致。開始前的還原點分支是
   `restore/pre-exit-experiments-2026-09-24`;放棄的話 revert 這批 commit,結論照第 5 點的格式記下來。
