...
5. **均線訊號的勝算怎麼驗證**:2026-09 這輪把「用 Monte Carlo 驗證過」從一句口頭敘述變成真的
   可重現的程式——「訊號」分頁的「策略回測」卡片,用 00631L/00675L 的真實歷史日報酬跑一次
   歷史回測,再用 block bootstrap 重抽樣跑蒙地卡羅(`walkTrend()`/`runSignalBacktest()`/
   `blockBootstrapSample()`/`runMonteCarlo()`,都在 `computeExposurePlan()` 附近)。
   **範圍刻意限定在只驗 `computeTrend()` 這套訊號本身的進出時機**,不含曝險目標、不含房貸
   利息——跟第 3 點的曝險目標是分開驗證的兩件事,不要混為一談。

   原本想用台灣加權指数的歷史日報酬自己合成槓桿路徑,但批次抓取指数歷史資料被 TWSE 的 WAF
   擋下(逐月請求抓到一半整個被標記,換頭、加 cookie、拉長間隔到分鐘級都沒用,連换其他資料源
   如 Yahoo Finance 都立刻被擋——這個環境對外的網路身分本身已經被多個金融資料源標記,不是
   單純調整抓取節奏能解決的),改用使用者自己提供的 `00631L.TW.csv`/`00675L.TW.csv`
   (Yahoo Finance 格式:Date/Open/High/Low/Close/Volume,約 2016 年中~現在)換算成日報酬,
   直接用真實 ETF 報酬還更準——不用自己假設費用率/追蹤誤差。**這兩份 CSV 只是一次性的資料
   來源,不是常態流程**,已內嵌成 `ETF_RET_00631L_RAW`/`ETF_RET_00675L_RAW` 這兩個字串常數,
   之後要更新/延伸年限,一樣得先想辦法拿到新的歷史資料(使用者手動提供,或找到不會被擋的
   資料源),不是重跑舊的抓取邏輯就會動。「歷史回測」只用 00631L 這條連續路徑;
   「蒙地卡羅」把兩檔的報酬合併當抽樣池。

   **實測發現一個不直覺但合理的結果,別看到就以為算錯**:單一條真實歷史回放時,訊號策略明顯
   贏過買進不動(期末資產、最大回落都更好);但蒙地卡羅重抽樣後,訊號策略的期末資產勝率反而
   偏低(多數合成路徑是買進不動贏),最大回落倒是穩定比較小。原因是這兩檔 ETF 上市以來剛好是
   台灣科技股非常強的十年多頭,少數几次崩盤都是 V 型急跌急彈——真實歷史裡訊號剛好躲過那幾次
   下跌,但 block bootstrap 打散時間順序之後,這種「跌了馬上彈」的幸運時序被稀釋掉,策略的
   「保護下檔」效果還在,但「贏過死抱」的效果大部分是這段特定歷史的時序運氣,不是必然。
